...
export function snapshotView(name: string, filters: FilterState, horizons: readonly string[], sort: SortKey, reverseLanes = false, showCovers = true): SavedView {
  return viewSchema.parse({ name, filters, horizons: [...horizons], sort, reverseLanes, showCovers });
}

/** Selections are sets; toggling them in a different order does not modify a view. */
type ViewSelection = Pick<SavedView, 'filters' | 'horizons' | 'sort'> & { reverseLanes?: boolean; showCovers?: boolean };

export function sameViewSelection(a: ViewSelection, b: ViewSelection): boolean {
  const set = (values: string[]) => [...new Set(values)].sort();
  const key = ({ filters: f, horizons, sort, reverseLanes = false, showCovers = true }: ViewSelection) => JSON.stringify([
    f.layout ?? 'board', timelineSettings(f.timeline),
    f.q, f.owner ?? null, f.product, set(f.stage), set(f.impact), set(f.effort),
    f.activity ?? null, f.visibility, f.hygiene, set(f.tags), f.group, set(horizons), sort, reverseLanes, showCovers,
  ]);
  return key(a) === key(b);
}

export const DEFAULT_VIEW: SavedView = {
  name: 'All priorities', filters: emptyFilters(), horizons: ['Now', 'Next', 'Later'], sort: 'manual', reverseLanes: false, showCovers: true,
};
