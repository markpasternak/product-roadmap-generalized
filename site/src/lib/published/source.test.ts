...
it('loads real Markdown and multiline YAML with absent optional collections', async () => {
  ...
  await mkdir(join(dir, 'content/items/studio'), { recursive: true });
  await mkdir(join(dir, 'content/prds'), { recursive: true });
  await writeFile(join(dir, 'content/items/studio/STUDIO-001-test.md'), markdown);
  await writeFile(join(dir, 'content/prds/Brief.md'), '---\ntitle: >-\n  A long\n  title\nvisibility: Public\n---\n# Body');
  const source = await loadContentSource(dir);
  expect(source.items[0]).toMatchObject({ id: 'studio/studio-001-test', data: { title: 'Title: quoted' }, body: '## Scope\nA useful change\n' });
...
it('rejects duplicate YAML keys and invalid schema before publishing', async () => {
  ...
  await writeFile(join(dir, 'content/items/test.md'), markdown.replace('id: STUDIO-001', 'id: STUDIO-001\nid: STUDIO-002'));
