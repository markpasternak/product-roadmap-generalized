...
export function contentRoutes(model: PublishedContent, base: string) {
  const routes = [
    { path: '', title: 'Product Roadmap', description: 'A fictional music portfolio roadmap across products, platforms and data, ordered by confidence rather than dates.' },
    ...PRODUCTS.map(product => ({ path: productSlug(product), title: `${product} · Product Roadmap`, description: `What's Now, Next and Later for ${product}, ordered by confidence rather than dates.` })),
    ...
    ...model.boardItems.map(item => ({ path: `item/${item.id}`, title: `${item.title} · Product Roadmap`, description: item.oneliner || `${item.product} roadmap item ${item.id}` })),
