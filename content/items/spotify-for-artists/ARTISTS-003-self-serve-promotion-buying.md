---
id: ARTISTS-003
title: Self-serve promotion buying
product: Spotify for Artists
horizon: Now
startDate: 2026-08-17
endDate: 2026-10-30
stage: Building
owner: Rafael Duarte
tags: promotion, monetisation, theme:creator-economy
impact: High
effort: High
order: 3
visibility: Public
external_visibility: Customer-safe
---

# Self-serve promotion buying

## One-liner
Buy and manage a promotional campaign for a release directly, without going through a sales conversation.

## Why it matters
Promotion today requires a minimum spend and a human on our side, which restricts it to major-label releases and leaves the long tail — where most of our catalogue growth is — with no way to spend money with us at all. Self-serve is how this becomes a business rather than a service line, and it is what independent artists have asked for most consistently. It also gives us a first-party demand source that does not compete with the Ads Platform.

## What ships
Campaign creation with budget and audience controls, projected reach before commitment, in-flight performance against that projection, billing and receipts, and shared reporting infrastructure with the Ads Platform rather than a parallel stack.

## Who it's for
Independent artists and small labels who cannot clear the current minimum spend.

## Target outcome
A large majority of promotion campaigns created without sales involvement, with the long tail becoming the volume driver.

## Open questions
- Where does the floor sit? Too low and we are running campaigns that cannot work; too high and we have rebuilt the current gate.
- Does self-serve promotion inventory compete with Ads Platform inventory for the same impressions, and who arbitrates?
